...
**NoteVideo** は、業務データ CSV を Google NotebookLM にアップロードし、AI が生成した解説動画 (MP4) をダウンロードできる Web アプリケーションです。

| 項目 | 内容 |
|------|------|
| リポジトリ名 | `notebooklm-csv-to-video` |
| 主要技術 | Python (FastAPI) + Next.js 16 + notebooklm-py |
...
flowchart TD
    ...
    Backend --> DB
    Backend --> Storage
    Backend -->|"asyncio タスク"| Runner
    Runner -->|"Playwright 経由"| NotebookLM
    Runner --> Storage
```
...
| GET | `/api/jobs` | ジョブ一覧 (`?status=` フィルタ対応) |
| GET | `/api/jobs/{id}` | ジョブ詳細 + ステップ進捗 |
| POST | `/api/jobs` | ジョブ作成 (CSV multipart アップロード) |
| GET | `/api/jobs/{id}/download` | MP4 ダウンロード |

**外部 API (`/api/v1/`)** — `X-API-Key` 認証必須

| メソッド | パス | 機能 |
|---------|------|------|
| POST | `/api/v1/jobs` | ジョブ作成 (JSON: Base64 または file_path で CSV 指定) |
...
| `backend/api_v1.py` | 外部 API ルーター | **完了** |
| `backend/runner.py` | バックグラウンドジョブ実行、NotebookLM 操作、Webhook 呼び出し | **完了** |
| `backend/database.py` | ジョブストア (インメモリ / MySQL 切り替え) | **完了** |
| `backend/storage.py` | ファイル保存 (ローカル / S3 切り替え) | **完了** |
...
| `/jobs/[id]` | ジョブ詳細 (5ステップ進捗 + ダウンロード) | **完了** |
...
| `useGenerateVideo()` | ジョブ作成 mutation | — |
...
| E2E 動作確認 | **完了** (CSV アップロード → MP4 生成 → ダウンロードまで確認済み) |
...
- [ ] プロジェクトルートで `git init`
- [ ] `.gitignore` に `uploads/`, `outputs/`, `.venv/`, `.env`, `*.mp4` 等を追加
...
- [ ] フロントエンドからジョブを投入し MP4 生成が完了することを確認
- [ ] HTTPS でアクセスできることを確認
- [ ] S3 から MP4 がダウンロードできることを確認
...
│   ├── main.py                        # アプリ本体、フロント用 API
│   ├── api_v1.py                      # 外部 API v1 ルーター
│   ├── runner.py                      # バックグラウンドジョブ実行
│   ├── database.py                    # ジョブストア (インメモリ/MySQL)
│   ├── storage.py                     # ファイル保存 (ローカル/S3)
