...
| `INVALID_ARGUMENT: could not resolve source` + **`401 Invalid Credentials` / `authError`** (often right after `Uploading tarball…`) | **Auth rejected** after upload (not the usual bucket **403**). In [`.github/workflows/cd-gcp.yml`](../.github/workflows/cd-gcp.yml), check the step **Verify Cloud Build access**: if **`gcloud builds list`** failed, fix WIF/SA first (Step 5, secrets **`GCP_WORKLOAD_IDENTITY_PROVIDER`** / **`GCP_SERVICE_ACCOUNT`**, SA not disabled). If **list succeeded** but submit still returns 401, retry the workflow; if it persists, open Support / Issue Tracker and include the **request id** from the `gcloud` log path printed in the job log. |
