...
- **Video jobs (`wait_completion`):** The JSON field `timeout` on `POST /api/v1/jobs` (and wiki-created jobs) must stay **≤ `cloud_run_worker_timeout`** on the Cloud Run **Job** worker, or the worker task can be terminated by GCP before polling finishes. Defaults in the app align with Terraform’s default worker timeout (`7200s`); if you increase `cloud_run_worker_timeout` in `terraform.tfvars`, set API environment variables **`VIDEO_JOB_TIMEOUT_MAX_SEC`** (and optionally **`VIDEO_JOB_TIMEOUT_DEFAULT_SEC`**) to the same number of seconds so clients cannot request a longer wait than the worker allows.
...
- **GCS signed URLs (`/api/videos/.../thumbnail`, `/stream`):** The API uses V4 signed URLs for private buckets. On Cloud Run, the runtime service account needs **`roles/storage.objectViewer`** (or equivalent) on the bucket *and* permission to sign via IAM: grant **`roles/iam.serviceAccountTokenCreator`** on **that same service account** to itself (`principal` = the runtime SA email). The app uses IAM SignBlob when ADC has no embedded private key ([`backend/storage.py`](../backend/storage.py) `_gcs_signed_url`).
- **Terraform:** With **`enable_gcs = true`**, Mitene Terraform already applies this for the default Compute Engine runtime SA (`PROJECT_NUMBER-compute@developer.gserviceaccount.com`): bucket IAM **`roles/storage.objectAdmin`**, SA self-binding **`roles/iam.serviceAccountTokenCreator`**, and enables **`iamcredentials.googleapis.com`** — see [`infra/terraform/gcs.tf`](../infra/terraform/gcs.tf). Use `terraform apply` so these resources exist; if Cloud Run uses a **custom** service account, add equivalent IAM for that SA (bucket + self Token Creator) or align the service to use the same SA Terraform configures.
