...
async def run_audio_job(
    *,
    ...
    try:
        ...
        prompt = (
            f"あなたは聴き手に直接語りかける話者です。\n"
            f"以下のCSVデータは聴き手自身に関するデータです。そのデータを分析し、"
            f"聴き手本人に「あなたの〜は〜ですね」のように直接語りかける"
            f"音声読み上げ用の原稿を{lang_hint}で作成してください。\n\n"
            f"## ユーザーからの指示\n"
            f"{instructions}\n\n"
            f"## 原稿作成のルール\n"
            ...
            f"- 「このデータでは〜」ではなく「あなたは〜」「あなたの〜は〜」のように、"
            f"聴き手本人に語りかける表現を使ってください\n"
            f"- 冒頭は「今日はあなたのデータを見ていきましょう！」「あなたの結果が出ましたよ！」のように"
            f"聴き手本人に語りかける一言で始め、"
            f"末尾は「ぜひ参考にしてみてください」「いかがでしたか？」などで自然に締めくくってください\n"
